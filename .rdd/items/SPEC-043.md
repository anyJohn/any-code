...
- DEC-154 视觉能力检测：provider 配置加 `vision?: boolean`（缺省 false）。发视觉消息时模型无 vision → 图片块降级为 `[图片: 文件名]` 文本占位 + Warning 事件提示。`decided_by: human`（Q1=A）
...
- B-006 视觉降级：模型无 vision（DEC-154）→ parts 中 image_url 块替换为文本占位 + Warning。
...
- I-001 非视觉模型收到的消息永不包含 image_url 块（B-006 兜底）。
...
- AC-002 given 非视觉模型+贴图 when 发送 then 请求中无 image_url，含文本占位，事件流出现 Warning。
