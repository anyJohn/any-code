...
describe("createSnapshotService（AR-4）", () => {
    ...
    it.skipIf(!gitOn)("快照/列表/回滚往返：修改文件后可恢复到快照时点", () => {
        ...
        const s2 = svc.snapshot("write a.txt v2 + b.txt");
        ...
        svc.rollbackTo(s1!.id);
        expect(readFileSync(join(ws, "a.txt"), "utf-8")).toBe("v1");
    });

    it.skipIf(!gitOn)("回滚不存在的 id → 抛错；非法 id → 抛错（注入防护）", () => {
        const svc = createSnapshotService(ws);
        svc.snapshot("init");
        expect(() => svc.rollbackTo("deadbeefdeadbeef")).toThrow(/不存在/);
        expect(() => svc.rollbackTo("../etc/passwd")).toThrow(/非法快照 id/);
    });

    it.skipIf(gitOn)("git 不可用 → available false，snapshot 返回 null", () => {
        // 仅在无 git 环境执行（CI 兜底断言）
        const svc = createSnapshotService(ws);
        expect(svc.available()).toBe(false);
        expect(svc.snapshot("x")).toBeNull();
    });
});
