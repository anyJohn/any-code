...
let cfg = {};
try {
    cfg = process.env.ABILITY_CONFIG
        ? JSON.parse(process.env.ABILITY_CONFIG)
        : {};
} catch {
    cfg = {};
}
...
async function resolveCdpUrl(raw) {
    ...
    if (!res.ok)
        throw new Error(`CDP /json/list HTTP ${res.status}（${origin}）`);
    ...
    if (!page)
        throw new Error(
            `CDP 端点 ${origin} 无 page target（先在浏览器里打开任意页面）`
        );
...
async function connect() {
    ...
    const url = await resolveCdpUrl(cdpUrl);
        ...
        ws.onopen = () => {
            ...
            ws.onmessage = (ev) => {
                ...
                if (msg.id !== undefined) {
                    ...
                    if (msg.error)
                        p.reject(new Error(`CDP: ${msg.error.message}`));
...
async function content() {
    try {
        ...
        if (!text && !url)
            return textResult("(页面无内容——可能未导航或空白页)");
        return textResult(
            `URL: ${url}\n标题: ${title || "(无标题)"}\n\n${
                text || "(无可读文本)"
            }`
